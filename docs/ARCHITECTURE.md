...
| **Cloud Run · operator dashboard** | The observability UI shown here - plain-English "why", decision log, platform health check, spend and tests - is the same app, served from Cloud Run reading BigQuery. Operators get in the cloud exactly what the demo shows locally, with no data leaving the project. |
| **Cloud Scheduler** | Fires the `platform_qa` self-audit and the review-cadence sweep on a cron, publishing to Pub/Sub so they run as ordinary traced agents. The health check and owner nudges are scheduled infrastructure, not a person remembering to look. |
| **Looker Studio** | The analyst layer on the same BigQuery tables: ad-hoc joins of agent behaviour to revenue, for people who will not open the app. No extra auth surface, no data leaving the project. |
...
**Self-audit and reviews.** Cloud Scheduler triggers the `platform_qa` audit on a cron into Pub/Sub. It runs the same checks that gate CI - every agent owned, reviews on time, DLQ clean, eval gate green, fallback rate healthy, per-driver precision holding - posts a digest to Slack, and nudges the owner of any agent overdue for review. The dashboard's health-check and review panels read the same result, so "is the platform healthy?" is answerable in the cloud without opening a log.

**Proof of live inference.** Every model call records the model, its token counts and its cost, so the cost panel names the exact model and real spend when live, and reads $0 with nothing named when the deterministic fallback ran. Which path executed is observable, not asserted.
